...
class BaseInfluencer(ABC):
    ...
    def display_campaigns_participated(self):
        ...
        campaigns_display = f"{self.__class__.__name__} :) {self.username} :) participated in the following campaigns:\n"

        for campaign in self.campaigns_participated:
            reached_followers = self.reached_followers(campaign.CAMPAIGN_TYPE)
            campaign_info = f"  - Campaign ID: {campaign.campaign_id}, Brand: {campaign.brand}, Reached followers: {reached_followers}\n"
            campaigns_display += campaign_info

        return campaigns_display.rstrip()
